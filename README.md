...
- 新建武器按一级射程排序选址：火箭10格放内墙后1格、电磁炮6格放后2格、加特林3格放后3格，随出生侧镜像。第一层上下侧墙延伸至加特林所在列，使三座新建武器处于正面墙与侧墙之后；后方仍保留己方通道。候选被占或非法时尝试同层其他格及邻层。已有武器无法直接移动；当前不自动覆盖或重建已有升级武器。
- 第三夜前锁定三座武器升二级：每个一级武器会分配独立升级任务，即使暂时不足100金币也保留目标并继续筹资；尚未购买升级券的城墙/基地任务会让位，召唤令等进攻消费在三座武器全部二级前禁用。按任务书基础价总需求为300金币，实际使用 `weaponShopList` 实时报价。
