...
1. 两名工人优先完成规划城墙，再补齐3座武器；保留自救、开拓者任务和临夜就位优先级。首日白天不卖矿、不为收益采矿、不进行进攻购物，资金不足也不切回卖矿。
...
- 第二天起，满足任一条件就专程卖矿：可卖矿石按当前报价估值至少25金币、可卖数量至少12个、背包占用至少35%、角色血量低于60%，或进入白天第40回合。已在小贩旁时也可出售。卖矿目标持续到可卖矿石清空，并跨服务重启保存；往返及出售时间不足则暂停外出。
...
- 基地7格内至少4个敌方目标机器人，或基地低于60%血量且附近有机器人，视为高压力。此时选择有望击杀至少2个或造成至少200总伤害的密集区域使用炸弹；没有库存则尝试购买。夜间不让守卫离炮远行购物，同回合避免重复购买与重复轰炸同一批目标。
