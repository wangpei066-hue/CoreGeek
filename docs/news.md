...
- **传闻**：只累积原文；待解码时走宝藏 LLM。同一批原文成功解过就不再重问；等新传闻或召唤失败（地点/物品不对）再送。开启窗口认正文「第N天/日」附近的开启/松动/白昼（白昼只填白天 70 回合）；「原点之北/东 N公里」按 1 公里=1 格换算，纯方位不猜。听到日和当前日不算。LLM 把坐标或窗口留空时，本地按这两类原文补全。
