...
- **官方**：原文变化（且不是「无重大新闻」）时**固定送 1 次**矿价 LLM，排在传闻前面；当天最多 1 次。启发式暂时关闭。累积 `officialHistory`，落地时与已有 `oreEffects` **取并集**（进度确认不会丢掉更早推出的禁采日）；仅「恢复开采」类消息清空。
- **传闻**：只累积原文；待解码时走宝藏 LLM，用当天剩余额度（官方占用后最多约 2 次）。同一批原文成功解过就不再重问；等新传闻或召唤失败（地点/物品不对）再送。开启窗口只认正文里的「第N天开启」，听到日和当前日不算。
