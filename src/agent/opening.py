...
def weapon_slot_plan(state, base):
    """武器编位：两门火箭贴在背敌上角两边；电磁炮在院内靠上、错开迎敌内侧整列。

    迎敌内侧那一列是院内上下连通、以及正面前端墙的施工邻格，不能再放炮。
    """
    left, right, bottom, top = defense_bounds(state, base)
    direction = attack_direction(state, base)
    front_x = (right if direction == 1 else left) - direction
    rocket_rear, rocket_side, stand = rear_corner_layout(state, base)
    station = station_cells(base)
    occupied = station | {rocket_rear, rocket_side, stand}
    rail = (front_x - direction, top - 1)
    if rail in occupied or not _in_map(state, rail):
        rail = (front_x - direction, bottom + 1)
    slots = [
        ('rocket', rocket_rear),
        ('railgun', rail),
        ('rocket', rocket_side),
