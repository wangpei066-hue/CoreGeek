...
def make_treasure_prompt(state: MatchState, memory: NewsMemory) -> str:
    ...
    rules = (
        ...
        f"- now.gameDay / now.roundNo 只表示现在，禁止用来填开启窗口。每天{DAY_NIGHT_CYCLE}回合，白天70回合，游戏日从1起算。\n"
        f"- 开启窗口：正文写「第N天/日」且附近有开启/可召唤/解开/松动/可进/白昼时就要填。"
        f"整天：openFromRound=(N-1)*{DAY_NIGHT_CYCLE}，openToRound=N*{DAY_NIGHT_CYCLE}-1；"
        f"写了白昼/白天则只填白天：openFromRound=(N-1)*{DAY_NIGHT_CYCLE}，openToRound=(N-1)*{DAY_NIGHT_CYCLE}+69。"
        "「第五日白昼方可松动」→ 第5天白天。上古传说无日期、方位、听到日都不是开启时间。"
        "没有开启日原文时两个字段必须 null，禁止用 now、heardOnDay 或 previousHypothesis 填。\n"
        "- altarPos：数字坐标（如 (12,8) 或 x=12,y=8）直接用。"
        "「原点之北/南/东/西 N公里或N格」必须换算：原点(0,0)在左下，东=+x，北=+y，1公里=1格。"
        "例：原点之北三公里、之东三公里 → {\"x\":3,\"y\":3}。"
        "只有「西部」「东侧」等无数字方位 → 必须 null，不要猜地图中点。\n"
        ...
        "- 0.6–0.8：坐标和物品都有原文（含原点公里换算），窗口仍含糊。\n"
        "- 0.8–1.0：坐标、献祭物品、开启时间都能在正文中找到对应句子（含第N日松动/白昼）。\n"
        "ready=true 仅当：altarPos、items 都有正文依据（含原点公里换算），且 confidence≥0.7。否则 ready=false。"
